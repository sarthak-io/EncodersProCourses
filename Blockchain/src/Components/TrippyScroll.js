...
const TrippyScroll = () => {
  ...
  return (
    <div ref={targetRef} className="relative z-0 h-[800vh] bg-neutral-200">
      <div className="sticky top-0 h-screen  bg-bg-logo">
...
const generateSections = (count, color, rotate) => {
  ...
  const nextColor = color === "#070A0F" ? "#7179D3" : "#070A0F";
...
const Trippy = ({ rotate }) => {
  return (
    <motion.div className="absolute inset-0 overflow-hidden  bg-bg-black-dark">
      {generateSections(0, "#070A0F", rotate)}
    </motion.div>
  );
};
